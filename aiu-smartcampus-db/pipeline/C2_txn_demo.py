...
def ensure_demo_course(mysql, course_code="TXN501") -> Tuple[int, str]:
    ...
    Matches mysql/init/01_schema.sql:
      courses(department_id, lecturer_id, course_code, course_name, credits, created_at)
    """
    with mysql.cursor() as cur:
        # ensure department
        ...
        # ensure lecturer (courses.lecturer_id is NOT NULL)
        cur.execute("""
INSERT IGNORE INTO lecturers(department_id, first_name, last_name, email)
VALUES (%s,%s,%s,%s)
""", (dept_id, "Txn", "Demo", "txn.demo@aiu.test"))

        cur.execute("SELECT lecturer_id FROM lecturers WHERE email=%s", ("txn.demo@aiu.test",))
        lecturer_id = cur.fetchone()["lecturer_id"]

        # ensure course
        cur.execute("""
INSERT INTO courses(department_id, lecturer_id, course_code, course_name, credits)
VALUES (%s,%s,%s,%s,%s)
...
""", (dept_id, lecturer_id, course_code, "Advanced Transaction Systems (Demo)", 3))
...
def demo_saga(mysql, mongo_db, student_id: int, course_id: int, course_code: str) -> None:
    ...
    with mysql.cursor() as cur:
        ...
        """, (student_id, course_id))
        cur.execute("COMMIT;")
    print("MySQL committed enrollment OK")
